 ...
 */
public class GetPrivateLocationByIdAsyncTask extends AsyncTask<Void, Void, Location> {

    private String token;
    private Long id;

    public GetPrivateLocationByIdAsyncTask(String token, Long id) {
        this.token = token;
        this.id = id;
    }
    ...
    protected Location doInBackground(Void... voids) {
        ResponseEntity<ResponseSerializer<Location>> responseEntity = HttpRequestBuilder.getRestTemplateWithJacksonConverter()
                .exchange(new LocationsPathBuilder().buildPrivateLocationByIdPath(id), HttpMethod.GET, new HttpEntity<Location>(HttpRequestBuilder.getHttpHeadersWithHeader(token)), new ParameterizedTypeReference<ResponseSerializer<Location>>() {});
