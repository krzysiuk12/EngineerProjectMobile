 ...
 */
public class GetCoordinatesByAddressAsyncTask extends AsyncTask<Void, Void, ResponseSerializer<Location>> {

	private String token;
	private GoogleGeocodeSerializer googleGeocodeSerializer;

	public GetCoordinatesByAddressAsyncTask(String token, GoogleGeocodeSerializer googleGeocodeSerializer) {
		this.token = token;
	...
	protected ResponseSerializer<Location> doInBackground(Void... voids) {
		ResponseEntity<ResponseSerializer<Location>> responseEntity = HttpRequestBuilder.getRestTemplateWithJacksonConverter()
				.exchange(GooglePathBuilder.buildLocationCoordinatesByAddressPath(),
						HttpMethod.POST,
